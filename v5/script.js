...
function drawTargetSquare() {
    ...
    ctx.save();
    ctx.translate(targetSquare.x, targetSquare.y);
    ctx.rotate(targetSquare.angle || 0);

    // Dotted black outline
    ctx.strokeStyle = 'black';
    ctx.setLineDash([5, 5]);
    ctx.lineWidth = 2;
    ctx.strokeRect(-targetSquare.size / 2, -targetSquare.size / 2, targetSquare.size, targetSquare.size);
    ctx.setLineDash([]);

    // Gray corner dots
    const corners = [
        [-targetSquare.size / 2, -targetSquare.size / 2],
        [targetSquare.size / 2, -targetSquare.size / 2],
        [targetSquare.size / 2, targetSquare.size / 2],
        [-targetSquare.size / 2, targetSquare.size / 2]
    ];
...
function setDifficulty() {
    ...
    distanceDisplay.textContent = calculateDistanceToTarget();
    squareHistory = [];
    pivot = null;
...
canvas.addEventListener('click', function (e) {
    ...
    if (!pivot) {
        ...
        squareHistory.push({ ...square });

        let pivotPoint = { ...pivot };
        let targetSq = { ...lastGhostSquare };
        ...
        animateRotation(pivotPoint, targetSq.angle - square.angle, () => {
            square = { ...targetSq };
            moves++;
            draw();
